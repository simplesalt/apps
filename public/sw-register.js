          ...
          newWorker.addEventListener('statechange', () => {
            if (newWorker.state === 'installed') {
              if (navigator.serviceWorker.controller) {
                console.log('[SW-Register] New service worker installed, will activate on next page load');
              } else {
                console.log('[SW-Register] Service worker installed and activated');
              }
            }
          });
        });
              ...
              return new Promise((resolve, reject) => {
                messageChannel.port1.onmessage = (event) => {
                  if (event.data.success) {
                    ...
                    console.error('[SW-Register] Failed to reload routing config:', event.data.error);
...
console.log('[SW-Register] Service worker support:', 'serviceWorker' in navigator);